-- ============================================================================
-- V1.28: Security Scan Push Notifications
-- ============================================================================
-- Adds push notifications triggered on QR scan outcomes:
--   access_scan_approved  → entry/exit confirmed to the resident
--   access_scan_warning   → "Was this you?" alert when already checked in
--   access_scan_denied    → access denied alert to the resident
--
-- Uses existing tenant_notifications + tenant_push_deliveries pipeline.
-- ============================================================================

-- ── Extend the notification type check constraint ─────────────────────────────
...
alter table app.tenant_notifications
  ...
  check (
    type in (
      ...
      'access_scan_approved',
      'access_scan_warning',
      'access_scan_denied'
    )
  );

-- ── Update trigger to queue push deliveries for new types ─────────────────────
