-- ============================================================================
-- V 1 16: Fix Payment RPC Table Names
-- ============================================================================
-- Description:
--   Fixes the 'relation "app.tenancies" does not exist' error by correcting 
--   the table references (app.unit_tenancies) and column names (tenant_user_id).
--   Restores the robust tenancy check from V 1.12 while keeping Setup ID.
-- ============================================================================
...
begin
  ...
  -- Restored from V 1.12 for robustness: unit_tenancies OR occupancy snapshot
  select exists (
    select 1
  ...
  if not v_has_tenancy then
    raise exception 'Access denied: no confirmed tenancy for this unit';
  end if;
  ...
  with setups as (
    select 
      ...
      coalesce(nullif(trim(s.account_reference_hint), ''), v_unit_label) as account_reference,
...
grant execute on function app.get_active_payment_setup_for_tenant(uuid) to authenticated;
