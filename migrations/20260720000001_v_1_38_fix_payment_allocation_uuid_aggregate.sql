-- ============================================================================
-- V1.38: Fix max(uuid) does not exist in payment allocation triggers
-- ============================================================================
-- Purpose
--   Two trigger functions on app.payment_allocations both called max() on a
--   uuid column to check "is every allocation for this payment/lease
--   pointing at the same unit / lease agreement?". Postgres has no built-in
--   max()/min() aggregate for uuid (only comparison operators), so both
--   functions raised:
--     ERROR: function max(uuid) does not exist
--   on every insert into app.payment_allocations — including the app's
--   normal "record a payment" flow, not just bulk/demo inserts.
--
--   Fixed by casting through text for the aggregate, then back to uuid
--   (text has a native max()). Behavior is unchanged: the whole point of
--   both max() calls is "is there exactly one distinct value" (already
--   checked separately via count(distinct ...) / an inequality comparison),
--   not to make use of a specific ordering over uuid values.
--
-- Affected functions (both defined in
--   20260409102000_v_1_09_revenue_payments_core.sql):
--   - app.refresh_payment_record_allocation_state(uuid)
--   - app.validate_payment_allocation() [trigger]
-- ============================================================================
